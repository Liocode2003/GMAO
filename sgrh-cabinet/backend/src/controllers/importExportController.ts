...
// Colonnes requises pour qu'un import soit considéré valide
...
// Mapping des libellés French → valeur en base
...
function validateRow(row: Record<string, unknown>, rowIndex: number): { errors: string[] } {
  ...
  if (row.marital_status && !VALID_MARITAL.includes(String(row.marital_status)))
    errors.push(`Situation matrimoniale invalide ligne ${rowIndex} (valeurs : Célibataire, Marié(e), Divorcé(e), Veuf/Veuve) : "${row.marital_status}"`);
...
export const parseImport = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Détecter les colonnes manquantes ou inconnues
    const missingColumns = REQUIRED_COLUMNS.filter(col => !headers.includes(col));
    const unknownColumns = headers.filter(h => h && !EXCEL_COLUMNS[h]);
    const expectedColumns = Object.keys(EXCEL_COLUMNS);
    ...
    for (let i = 2; i <= sheet.rowCount; i++) {
      ...
      if (data.matricule) {
        ...
        if (r.rows[0]) {
          errors.push(`Matricule "${data.matricule}" déjà existant en base`);
          isDuplicate = true;
        }
      }
      if (data.email) {
        ...
        if (r.rows[0]) {
          errors.push(`Email "${data.email}" déjà utilisé en base`);
          isDuplicate = true;
        }
      }
